...
const MODULE_GUIDE = {
  ...
  "cp": ["NeoCloud OS Control Plane — SKT Core Engine",
    "포털/외부 요청을 받아 NVIDIA·파트너 인프라 도메인(Compute·SDN·Storage·Shared)을 오케스트레이션하는 SKT 자체 계층. 멀티테넌시·과금·격리 정책·승인 워크플로 등 상품화 기능을 보유하고, 하부 플랫폼의 네이티브 관리 기능은 중복 구현하지 않는다."],
  ...
  "cp-fulfill": ["Tenant Fulfillment — saga 오케스트레이터",
  ...
  "cp-provision": ["BM Provisioning — 베어메탈 프로비저닝",
    "ComputeAdapter 경유 노드 프로비저닝 — reserve(gRPC)→provision: BMC(Redfish) 전원·부트오더 → DPU-DHCP IP 할당 → PXE 테넌트 이미지 → cloud-init. job 폴링 수렴, 실패 노드는 quarantine 격리."],
  ...
  "cp-model": ["Resource & Service Model — 단일 리소스 모델",
    "Factory▸Block(층)▸DU▸SU▸Rack▸Tray▸GPU 토폴로지 SoT · NodeInstance 상태머신 미러 · reconcile로 외부 SoT(NICo)와 정합성 감사(GHOST/ORPHAN/STATE_MISMATCH) · 장비 헬스 집계."],
  "cp-policy": ["Policy Orchestration — 정책 결정 · 배치 엔진",
    "배치 엔진 = 주문한 랙을 \"어느 사이트·어느 SU·어느 랙에 둘지\" 정하는 규칙 엔진. ① 랙 단위 판매 — NVLink는 랙(NVL72) 안에서만 닫히므로 랙을 쪼개지 않는다 ② best-fit — 한 SU에 들어가면 남는 공간이 가장 작은 SU를 골라 파편화 최소화 ③ 넘치면 여러 SU로 분할(spill)하되 같은 사이트 안에서만 — 가산·안산 간 IB가 없어 클러스터가 사이트를 못 넘는다 ④ dedicated 테넌트가 점유한 SU에는 타 테넌트 배치 금지(양방향). 그 외 격리 티어→DPU 모드 도출, 4계층 격리 검증 리포트(acceptance 게이트)도 이 모듈 소관."],
  "cp-obs": ["Observability — 관측 취합",
    "시스템 세부 트레이스 버스(전 채널 메시지 페이로드) · NICo 벌크 헬스·DCGM 시계열(240틱) · XID/장애 이벤트 · 물리+논리 헬스 단일 SoT(CAP05) — 실배치는 OTel/NATS로 전환 예정."],
  "cp-sla": ["SLA Management — SLA/SLO",
    "프로비저닝 리드타임 자동 산출(주문 이력) · 가용성(in_service 비율) · NVIDIA SLA 타깃(관리면 99.95% financially-backed, 스토리지 QoS) 측정 체계 — 로드맵."],
...
const STAGE_GUIDE = {
  ...
  validated: ["validated — 정책 검증 + 배치",
    "테넌트/블루프린트/규격 검사 후 배치 실행 — 단일 사이트 내 best-fit(랙 단위·NVL 무결성), dedicated SU 격리 반영. 용량 부족 시 rejected(사이트별 가용 내역 포함)."],
